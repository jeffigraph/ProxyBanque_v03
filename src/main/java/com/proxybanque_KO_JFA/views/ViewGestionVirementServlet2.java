...
//			long idClientCredit = Long.parseLong(request.getParameter("idclientcrediteur"));
			...
			try {
				...
				// Client crediteur =
				// srvcEng.getServiceGestionClient().getClient(idClientCredit);
				Client debiteur = srvcEng.getServiceGestionClient().getClientByNumCompte(compteDebit);
				Client crediteur = srvcEng.getServiceGestionClient().getClientByNumCompte(compteCredit);

				Compte compteDebiteur = debiteur.getCompteByNum(compteDebit);
				Compte compteCrediteur = crediteur.getCompteByNum(compteDebit);

				// System.out.println("debiteur: " + debiteur.getSolde() + " crediteur:" +
				// crediteur.getSolde());

				if (null != compteDebiteur && null != compteCrediteur && srvcEng.getServiceVirement()
						.faireVirement(debiteur, compteDebiteur, crediteur, compteCrediteur, montant)) {
